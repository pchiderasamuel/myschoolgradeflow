          ...
          <main className="flex-1 overflow-y-auto p-4 md:p-6 lg:p-8" onClick={()=>menuOpen&&setMenuOpen(false)}>
            <div className="max-w-5xl mx-auto space-y-6 pb-8">
                ...
                <Card className="overflow-hidden">
                  ...
                  <div className="p-6 space-y-5">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <Sel label="Class" value={scoreForm.studentClass} onChange={(e: any)=><Sel label="Class" value={scoreForm.studentClass} onChange={(e: any)=>setScoreForm(f=>({...f,studentClass:e.target.value,subject:""}))}><option value="">Select class</option>{(auth.user?.assignedClasses?.length?auth.user.assignedClasses:ALL_CLASSES).map((c: string)=><option key={c}>{c}</option>)}</Sel>}><option value="">Select class</option>{(auth.user?.assignedClasses?.length?auth.user.assignedClasses:ALL_CLASSES).map((c: string)=><option key={c}>{c}</option>)}</Sel>
              ...
              {activeTab==="reports"&&can("viewReports")&&(!activeReport?<>
                <div><h1 className="text-2xl font-black text-slate-900 uppercase">Reports</h1><p className="text-sm text-slate-400">{filteredStudents.length} students found</p></div>
                <div className="flex flex-col sm:flex-row gap-3"><div className="relative flex-1"><Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400"/><input value={rpSearch} onChange={(e: any)=>setRpSearch(e.target.value)} placeholder="Search student…" className="w-full pl-9 pr-4 py-3 bg-white border-2 border-slate-100 rounded-xl text-sm font-semibold focus:border-primary outline-none shadow-sm"/></div><select value={rpClass} onChange={(e: any)=>setRpClass(e.target.value)} className="px-4 py-3 bg-white border-2 border-slate-100 rounded-xl text-sm font-semibold focus:border-primary outline-none shadow-sm"><option value="All">All Classes</option>{ALL_CLASSES.map(c=><option key={c}>{c}</option>)}</select></div>
                {filteredStudents.length===0?<EmptyState icon={FileText} title="No students found" subtitle="Add scores to see students"/>:<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">{filteredStudents.map((s: any)=><button key={s.id} onClick={()=>openReport(s)} className="p-5 bg-white border-2 border-slate-100 rounded-2xl flex items-center justify-between text-left group hover:border-blue-400 hover:shadow-md transition-all"><div><p className="font-black text-sm uppercase text-slate-900">{s.name}</p><p className="text-xs font-bold text-slate-400 mt-0.5">{s.class}</p></div><FileText size={18} className="text-slate-300 group-hover:text-primary transition-colors"/></button>)}</div>}
              </>:
