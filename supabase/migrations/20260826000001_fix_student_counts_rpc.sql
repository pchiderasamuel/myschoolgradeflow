-- =====================================================================
-- 20260826000001_fix_student_counts_rpc.sql
-- Fix get_student_counts_by_school to prevent double counting
-- and accurately calculate total active students per school.
-- =====================================================================
...
BEGIN
  ...
  SELECT 
    s.id AS school_id, 
    GREATEST(
      COALESCE(sub_json.cnt, 0)::int, 
      COALESCE(sub_rel.cnt, 0)::int, 
      COALESCE(s.current_students, 0)::int
    ) AS student_count
